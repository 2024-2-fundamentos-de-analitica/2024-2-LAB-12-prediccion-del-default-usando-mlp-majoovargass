...
def outputCreation(dicout):
        if os.path.exists(dicout):
            for file in glob(f"{dicout}/*"):
                os.remove(file)
            os.rmdir(dicout)
        os.makedirs(dicout)
...
def pregunta01():
    ...
    def cleanse(df):
        ...
        dfp = dfp.loc[dfp["MARRIAGE"] != 0]
        dfp = dfp.loc[dfp["EDUCATION"] != 0]
        dfp["EDUCATION"] = dfp["EDUCATION"].apply(lambda x: 4 if x >= 4 else x)
        return dfp.dropna()
        
    test_df, train_df = [cleanse(dfp) for dfp in loadData("files/input")]
